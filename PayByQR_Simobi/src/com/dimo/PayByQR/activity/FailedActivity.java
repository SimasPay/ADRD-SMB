...
public class FailedActivity extends AppCompatActivity {
    ...
    private void onBtnOKClick(int reqCode){
        if(0 != reqCode){
            ...
            if(Constant.REQUEST_CODE_ERROR_INVALID_QR == reqCode){
                closeSDK(listener.callbackInvalidQRCode());
            }else if(Constant.REQUEST_CODE_ERROR_CONNECTION == reqCode){
                closeSDK(false);
            }else if(Constant.REQUEST_CODE_ERROR_UNKNOWN == reqCode){
            ...
            }else if(Constant.REQUEST_CODE_ERROR_PAYMENT_FAILED == reqCode){
            ...
            }else if(Constant.REQUEST_CODE_ERROR_TIME_OUT == reqCode){
                boolean isClose = listener.callbackTransactionStatus(Constant.ERROR_CODE_TIME_OUT, errorDetail);
                if(PayByQRSDK.getModule() == PayByQRSDK.MODULE_IN_APP || PayByQRSDK.getModule() == PayByQRSDK.MODULE_LOYALTY) closeSDK(true);
                else closeSDK(isClose);
            }else if(Constant.REQUEST_CODE_ERROR_AUTHENTICATION == reqCode){
